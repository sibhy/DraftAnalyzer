# All this code is from: https://www.cs.cmu.edu/~112/notes/notes-animations-part1.html

# The following changes were made:
# - disabled resizing(https://coderslegacy.com/python/problem-solving/disable-resizing-in-tkinter/)
# - added custom icon(https://www.geeksforgeeks.org/change-icon-for-tkinter-messagebox/)
# - disabled exception of calling loadImage in redrawAll so app resets background image based on selection
...
class App(object):
    ...
    def updateTitle(app):
        ...
        # picture is from: https://cdn0.iconfinder.com/data/icons/sports-icons/110/American-Football-512.png
        # guide to how I changed it: https://www.geeksforgeeks.org/change-icon-for-tkinter-messagebox/
        # this path is location specific, so will start as commented and can be
        # uncommented when the path is updated
        # app._root.iconbitmap(r'C:\Users\sibhy\OneDrive\Fall 2020\15112 Files\TermProject-f20-SibhyRajesh\football.ico')
    ...
    def getQuitMessage(app):
        ...
        return f"*** Closing {app._title}.  Bye! ***\n"

    def _showRootWindow(app):
        root = app._root
        root.update(); root.deiconify(); root.lift(); root.focus()

        # https://coderslegacy.com/python/problem-solving/disable-resizing-in-tkinter/
        # root.resizable(False, False)
        # didn't really know where to put this, but didn't want user to resize
        # the window cause the images aren't built for that 
